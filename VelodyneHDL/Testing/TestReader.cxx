...
typedef bool (*TestActionFunction)(vtkSmartPointer<vtkPolyData>&, std::istream&);
...
bool testFrameCount(vtkSmartPointer<vtkPolyData>& frame, std::istream& is)
{
  ...
  {
    std::cerr << "frame_count: expected " << expectedCount << ", got " << actualCount << std::endl;
    return false;
  }
...
bool testPointCount(vtkSmartPointer<vtkPolyData>& frame, std::istream& is)
{
  ...
  if (actualCount != expectedCount)
  {
    std::cerr << "point_count: expected " << expectedCount << ", got " << actualCount << std::endl;
    return false;
  }
...
bool testPointValues(vtkSmartPointer<vtkPolyData>& frame, std::istream& is)
{
  ...
  bool isDual;
  is >> pointId >> expectedPosition[0] >> expectedPosition[1] >> expectedPosition[2] >>
    expectedIntensity >> isDual;
  if (isDual)
  {
  ...
  vtkDataArray* const intensityData = frame->GetPointData()->GetArray("intensity");
  vtkDataArray* const dualDistanceFlagData = frame->GetPointData()->GetArray("dual_distance");
  vtkDataArray* const dualIntensityFlagData = frame->GetPointData()->GetArray("dual_intensity");
  ...
  {
    std::cerr << "point_values: frame intensity data missing or truncated" << std::endl;
  }
  ...
  const double actualIntensity = intensityData->GetComponent(pointId, 0);
  ...
  if (!compare(expectedPosition, actualPosition) || !compare(expectedIntensity, actualIntensity) ||
  ...
  {
    std::cerr << "pount_values: expected " << expectedPosition[0] << ' ' << expectedPosition[1]
              << ' ' << expectedPosition[2] << ' ' << expectedIntensity << ' ' << isDual << ' ';
    if (isDual)
    {
      std::cerr << expectedRelativeDistance << ' ' << expectedRelativeIntensity;
    }
    std::cerr << ", got " << actualPosition[0] << ' ' << actualPosition[1] << ' '
              << actualPosition[2] << ' ' << actualIntensity << ' ' << isDual << ' ';
    if (isDual)
    {
      std::cerr << actualRelativeDistance << ' ' << actualRelativeInteensity;
    }
    std::cerr << std::endl;
    return false;
  }
...
int main(int argc, char* argv[])
{
  ...
  const TestActionFunctionMap::const_iterator invalidCommand = testActions.end();

  if (argc < 3)
  {
    std::cout << "Usage: " << argv[0] << " <pcap file> <calibration> [<expect file>]" << std::endl;
    return 1;
  }
  ...
  if (testScriptFilename.empty())
  {
    ...
    for (int i = 0; i < frames.size(); ++i)
    {
      ...
      std::cout << "  frame " << frameId
                << " has points: " << (frame ? frame->GetNumberOfPoints() : 0) << std::endl;
    }
  }
  else
  {
    ...
    for (;;)
    {
      ...
      // look up command
      const TestActionFunctionMap::const_iterator iter = testActions.find(command);
